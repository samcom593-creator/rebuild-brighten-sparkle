...
/** Center-crop an image to a 512×512 square JPEG via Canvas */
async function autoCrop(file: File): Promise<Blob> {
...
export function AvatarUpload({ userId, currentAvatarUrl, fullName, onAvatarChange }: AvatarUploadProps) {
  ...
  const handleFileSelect = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    setUploading(true);
    try {
      // Auto-crop to centered 512x512 square
      const croppedBlob = await autoCrop(file);
  ...
  const handleDelete = async () => {
    ...
    try {
      ...
      toast({ title: "Delete Failed", description: "Could not remove your photo. Please try again.", variant: "destructive" });
    } finally {
      setDeleting(false);
    }
  };
    ...
    <div className="flex flex-col items-center gap-4">
      ...
      <div className="flex gap-2">
        <Button type="button" variant="outline" size="sm" onClick={() => fileInputRef.current?.click()} disabled={uploading}>
          {uploading ? (<><Loader2 className="h-4 w-4 mr-2 animate-spin" />Uploading...</>) : (<><Camera className="h-4 w-4 mr-2" />{currentAvatarUrl ? "Change Photo" : "Upload Photo"}</>)}
        </Button>
        {currentAvatarUrl && (
      ...
      <p className="text-xs text-muted-foreground text-center">JPG, PNG or GIF. Max 5MB. Auto-cropped to square.</p>
    </div>
  );
}
