  ...
  return (
    <section className="relative overflow-hidden rounded-2xl border border-[#C9A961]/25 bg-[#0A0A0A] text-white shadow-[0_24px_80px_rgba(0,0,0,0.24)]">
      ...
      <div className="relative p-4 sm:p-6">
        <div className="flex flex-col gap-4 lg:flex-row lg:items-start lg:justify-between">
          <div className="min-w-0 max-w-3xl">
            <div className="mb-3 flex flex-wrap items-center gap-2">
              ...
              {updatedLabel && <span className="text-[11px] text-white/45">Updated {updatedLabel}</span>}
            </div>
            <h1 className="text-balance text-2xl font-black tracking-tight sm:text-3xl lg:text-4xl">{title}</h1>
            <p className="mt-2 max-w-2xl text-sm leading-relaxed text-white/60 sm:text-base">{subtitle}</p>
          </div>
            ...
            const body = (
              <>
                <div className="flex items-start justify-between gap-2">
                  ...
                  {metric.onClick && <ArrowUpRight className="h-3.5 w-3.5 text-white/25 transition-colors group-hover:text-[#C9A961]" />}
                </div>
                ...
                <p className="mt-1.5 text-[11px] font-bold uppercase tracking-wide text-white/75">{metric.label}</p>
                <p className="mt-0.5 line-clamp-2 text-[10px] leading-relaxed text-white/40">{metric.detail}</p>
              </>
            );
            const classes = cn(
        ...
        <div className="mt-4 flex items-center gap-2 border-t border-white/10 pt-3 text-[11px] text-white/45">
