  ...
  return (
    <div className={cn("space-y-6 p-4 md:p-6 animate-in fade-in duration-200", className)}>
      ...
      {variant === "dashboard" && (
        <>
          <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
            {[...Array(4)].map((_, i) => (
              <div key={i} className="h-24 rounded-md bg-muted/40 animate-pulse" style={{ animationDelay: `${i * 75}ms` }} />
            ))}
          </div>
          <div className="grid grid-cols-1 lg:grid-cols-3 gap-4">
            <div className="lg:col-span-2 h-80 rounded-md bg-muted/40 animate-pulse" />
            <div className="h-80 rounded-md bg-muted/40 animate-pulse" style={{ animationDelay: "150ms" }} />
          </div>
      ...
      {variant === "table" && (
        <>
          ...
          <div className="grid grid-cols-2 md:grid-cols-5 gap-3">
            {[...Array(5)].map((_, i) => (
              <div key={i} className="h-20 rounded-md bg-muted/40 animate-pulse" style={{ animationDelay: `${i * 50}ms` }} />
            ))}
          </div>
      ...
      {variant === "list" && (
        <div className="space-y-2">
          {[...Array(8)].map((_, i) => (
            <div key={i} className="h-20 rounded-md bg-muted/40 animate-pulse" style={{ animationDelay: `${i * 60}ms` }} />
          ))}
        </div>
      )}
      ...
      {variant === "cards" && (
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
          {[...Array(9)].map((_, i) => (
            <div key={i} className="h-48 rounded-md bg-muted/40 animate-pulse" style={{ animationDelay: `${i * 60}ms` }} />
          ))}
        </div>
      )}
      ...
      {variant === "detail" && (
        <div className="grid grid-cols-1 lg:grid-cols-[1fr_320px] gap-6">
          <div className="space-y-4">
            ...
            <div className="h-64 rounded-md bg-muted/30 animate-pulse" />
            <div className="h-48 rounded-md bg-muted/30 animate-pulse" />
          </div>
          <div className="space-y-4">
