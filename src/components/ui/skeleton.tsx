  ...
  return (
    <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
      {Array.from({ length: count }).map((_, i) => (
        // stable-key-allow:skeleton — deterministic count-based grid skeleton, no reorder
        <div key={i} className="space-y-3 rounded-md border border-border/50 p-4">
...
function StatSkeleton({ count = 4 }: { count?: number }) {
  return (
    <div className="grid grid-cols-2 lg:grid-cols-4 gap-3">
      {Array.from({ length: count }).map((_, i) => (
        // stable-key-allow:skeleton — deterministic count-based grid skeleton, no reorder
        <div key={i} className="space-y-2 rounded-md border border-border/50 p-4">
