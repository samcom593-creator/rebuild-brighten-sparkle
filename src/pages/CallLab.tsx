...
export default function CallLab() {
  ...
  const sessions = useQuery({ queryKey: ["call-lab", "sessions", user?.id], enabled: Boolean(user?.id), queryFn: async () => { const { data, error } = await supabase.from("call_lab_sessions").select("id,scenario_id,status,mode,provider,created_at,scorecard,scenario_snapshot").eq("user_id", user!.id).order("created_at", { ascending: false }).limit(25); if (error) throw error; return (data ?? []) as unknown as SessionRow[]; } });

  const bests = useMemo(() => { const m: Record<string, { best: number | null; last: number | null; runs: number }> = {}; for (const s of sessions.data ?? []) { if (s.provider === "demo") continue; const sc = s.scorecard?.overallScore ?? null; const e = (m[s.scenario_id] ??= { best: null, last: null, runs: 0 }); e.runs += 1; if (sc !== null) { if (e.last === null) e.last = sc; if (e.best === null || sc > e.best) e.best = sc; } } return m; }, [sessions.data]);
  ...
  return (
    <div className="space-y-6">
      <PageHeader eyebrow="Training" eyebrowIcon={<Mic className="h-4 w-4" aria-hidden />} title="Call Lab" subtitle="Practice a real sales conversation. Choose a prospect, speak or type your responses, then review specific feedback and retry your weakest objection." actions={<ModeToggle mode={mode} onChange={setMode} />} />
      <p className="text-sm text-muted-foreground">{mode === "coach" ? "Coach mode gives you prompts for handling the prospect’s objections during the call." : "Practice mode keeps coaching until the report so you can test yourself."} No real customer is called. Only the practice transcript is saved.</p>
      ...
      <section className="grid gap-4 md:grid-cols-2 xl:grid-cols-3">
        ...
        {scenarios.isError && <div className="space-y-2"><p className="text-sm text-destructive">Scenarios could not load.</p><Button variant="outline" onClick={() => void scenarios.refetch()}>Retry scenarios</Button></div>}
        {!scenarios.isLoading && !scenarios.isError && scenarios.data?.length === 0 && <p className="text-sm text-muted-foreground">No practice scenarios are currently published.</p>}
      </section>
      <section>
        <h2 className="mb-2 text-sm font-medium uppercase tracking-wide text-muted-foreground">Recent calls</h2>
        {sessions.isError && <Button variant="outline" onClick={() => void sessions.refetch()}>Retry loading recent calls</Button>}
        ...
        <ul className="divide-y rounded-xl border">
          {sessions.data?.map((s) => { const score = s.scorecard?.overallScore ?? null; const done = s.status === "complete"; const report = done || ["ending", "evaluating", "failed"].includes(s.status); return (
            <li key={s.id}>
...
function MicCheck({ onDemo, demoReady }: { onDemo: () => void; demoReady: boolean }) {
  ...
  return (
    <div className="flex flex-wrap items-center gap-3 rounded-xl border px-4 py-3 text-sm">
      ...
      <span className="text-muted-foreground">{state === "ok" ? "Microphone works." : state === "denied" ? "Microphone blocked. Allow it in the browser site settings." : speechRecognitionSupported() ? "Chrome or Edge, headphones on, quiet room." : "Voice recognition is unavailable here. Use typed practice or watch the demo."}</span>
    </div>
  );
}
