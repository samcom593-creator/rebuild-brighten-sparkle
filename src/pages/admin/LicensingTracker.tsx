            ...
            <Card key={s.key} className={cn("min-w-[260px] flex-shrink-0", s.color)}>
              ...
              <CardContent className="p-2 space-y-1.5 max-h-[60vh] overflow-y-auto">
                ...
                {students.slice(0, visibleCount).map(st => (
                  <div key={st.id} className="rounded-md border border-border bg-white/[0.03] p-2.5 text-xs">
            ...
            <tbody>
              {rows.map(r => (
                <tr key={r.id} className="border-t border-border">
                  ...
                  <td className="p-3 text-xs text-slate-600 dark:text-slate-300">{r.email ?? r.phone ?? "—"}</td>
                </tr>
              ))}
            </tbody>
          </table>
...
export default function LicensingTracker() {
  return (
    <div className="min-h-screen p-4 lg:p-6 max-w-[1600px] mx-auto space-y-4 ops-surface ops-fade-in">
      <PageHeader
        accent="amber"
        eyebrow="Admin · Licensing"
