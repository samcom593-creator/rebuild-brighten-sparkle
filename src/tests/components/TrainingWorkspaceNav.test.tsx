...
function renderNav(pathname = "/dashboard/training/library") {
  return render(
...
describe("TrainingWorkspaceNav", () => {
  ...
  it("gives agents every learning section without staff controls", () => {
    renderNav();
    expect(screen.getByRole("link", { name: "Training home" })).toHaveAttribute(
      "href",
    ...
    expect(screen.queryByRole("link", { name: "Weekly leaderboard" })).not.toBeInTheDocument();
  });

  it("gives training staff the AgentCloud operating sections", () => {
    auth.isVa = true;
    renderNav("/dashboard/training/progress");
  ...
  it("keeps old recruiting bookmarks visibly anchored in the canonical workspace", () => {
    renderNav("/dashboard/recruiting/training/sales-course");
    expect(screen.getByRole("link", { name: "Field course" })).toHaveAttribute(
      "aria-current",
      "page",
    );
  });
