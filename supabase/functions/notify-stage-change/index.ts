...
const STAGE_INFO = {
  onboarding: {
    ...
    emoji: "📋",
    nextSteps: ["Complete your contracting paperwork", "Gather required documents", "Schedule your onboarding call"],
  },
  training_online: {
    label: "In Course",
    description: "Product training & certification",
    emoji: "📚",
    nextSteps: ["Watch all training videos", "Pass the quiz assessments", "Complete the certification modules"],
  },
  in_field_training: {
    label: "In-Field Training",
    description: "Shadowing & ride-alongs",
    emoji: "🎯",
    nextSteps: ["Shadow experienced agents", "Practice presentations", "Attend daily meetings"],
  },
  evaluated: {
    label: "Live",
    description: "Active in the field",
    emoji: "🚀",
    nextSteps: ["Log your daily numbers", "Close deals", "Build your referral network"],
  },
};
...
serve(async (req) => {
  ...
  try {
    ...
    if (!profile?.email) {
      ...
      return new Response(
        JSON.stringify({ error: "Profile email not found" }),
        { status: 404, headers: { ...corsHeaders, "Content-Type": "application/json" } }
      );
    }
  ...
  <div style="max-width: 600px; margin: 0 auto; padding: 20px;">
    ...
    <div style="background: ${isPromotion ? "linear-gradient(135deg, #10b981 0%, #059669 100%)" : "linear-gradient(135deg, #3b82f6 0%, #2563eb 100%)"}; border-radius: 16px 16px 0 0; padding: 40px 30px; text-align: center;">
      ...
      <h1 style="color: white; margin: 0; font-size: 28px; font-weight: bold;">
        ${isPromotion ? "🎉 Congratulations!" : "Stage Update"}
      </h1>
    ...
    <div style="background: #1a1a1a; padding: 30px; border-radius: 0 0 16px 16px; border: 1px solid #333; border-top: none;">
      <div style="text-align: center; margin-bottom: 24px;">
        <h2 style="color: white; margin: 0 0 8px 0; font-size: 24px;">
          You're now in: <span style="color: ${isPromotion ? "#10b981" : "#3b82f6"};">${stageInfo.label}</span>
        </h2>
      ...
      ${newStage === "evaluated" ? `
      <!-- Special CTA for Live agents -->
      <div style="margin-top: 24px; text-align: center;">
        <a href="https://rebuild-brighten-sparkle.lovable.app/agent-portal" 
           style="display: inline-block; background: linear-gradient(135deg, #10b981 0%, #059669 100%); color: white; font-size: 16px; font-weight: bold; padding: 14px 32px; border-radius: 12px; text-decoration: none;">
          🎯 Access Your Portal
        </a>
      </div>
      ` : ""}
      ...
      <div style="margin-top: 32px; padding-top: 24px; border-top: 1px solid #333; text-align: center;">
        <p style="color: #666; font-size: 12px; margin: 0;">
          Keep pushing forward! The Apex Financial team is here to support you.
        </p>
      </div>
    </div>
  </div>
