...
SELECT cron.schedule(
  ...
  $$SELECT net.http_post(
    url := 'https://xrzweoneiieddzxogewk.supabase.co/functions/v1/send-sam-morning-report',
    body := '{}'::jsonb,
...
SELECT cron.schedule(
  ...
  '0 14 * * *',
  $$SELECT net.http_post(
    url := 'https://xrzweoneiieddzxogewk.supabase.co/functions/v1/check-overdue-tasks',
    body := '{}'::jsonb,
    headers := jsonb_build_object('Content-Type', 'application/json', 'Authorization', 'Bearer ' || current_setting('app.settings.service_role_key', true))
  )$$
);
