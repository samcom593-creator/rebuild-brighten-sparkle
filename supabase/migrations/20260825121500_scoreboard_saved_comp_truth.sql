-- Estimated earnings must use the compensation edited on the producer account.
-- The previous function ignored agents.contract_percentage entirely and could
-- show 63% even after an admin saved the real number on Accounts.
...
begin
  ...
  select jsonb_build_object(
    ...
    'earnings', jsonb_build_object(
      ...
      'basis', 'Personal production multiplied by saved producer compensation; name-based compensation then 63% are explicit fallbacks'
    ),
...
comment on function public.scoped_production_scoreboard(date, date) is
  'Hierarchy-scoped personal/team production and estimated personal earnings. Saved agents.contract_percentage is authoritative; all production comes from v_production_unified.';
